...
def add_new_holiday_with_supplies(holiday_hash, season, holiday_name, supply_array)
  holiday_hash.each do |original_season, holidays|
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holidays|
    ...
    holidays.each do |holiday, supplies|
      ...
      formatted_holiday_name.map! {|item| item.capitalize}
      puts "  #{formatted_holiday_name.join(" ")}: #{supplies.join(", ")}"
    end
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each do |season, holidays|
    holidays.each do |name, supplies|
      supplies.each {|item| holiday_names << name if item == "BBQ"}
    end
  end
