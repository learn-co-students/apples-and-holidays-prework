...
def add_supply_to_memorial_day(holiday_hash, supply)
  ...
  holiday_hash.collect do |season, holidays|
    if (season == :spring)
      holidays.collect do |name, supplies|
        supplies << supply
      end
    end
  end
...
def add_new_holiday_with_supplies(holiday_hash, season, holiday_name, supply_array)
  ...
  holiday_hash.collect do |hseason, holidays|
    if hseason == season
...
def camel_case(symbol)
  symbol.to_s.split("_").collect {|item| item.capitalize!}.join(" ")
end
...
def all_supplies_in_holidays(holiday_hash)
  ...
  holiday_hash.each do |season, holidays|
    ...
    holidays.each do |name, supplies|
      puts "  #{camel_case(name)}: #{supplies.join(", ")}"
      # print "  #{name.to_s.sub!("_", " ").capitalize}:"
    end
  end
end
