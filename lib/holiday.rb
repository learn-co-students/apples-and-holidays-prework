...
def all_supplies_in_holidays(holiday_hash)
  ...
  def capitalize_key(key)
    ...
    key_array.each do |item|
      capitalized_key.push(item.capitalize!)
    end
    capitalized_key.join(" ")
  end

  holiday_hash.each do |season_key, season_hash|
    season_holidays = holiday_hash[season_key]
    season_string = capitalize_key(season_key)
    puts "#{season_string}:"

    season_holidays.each do |holiday_key, holiday_supplies_array|
      supplies_string = holiday_supplies_array.join(", ")
      holiday_string = capitalize_key(holiday_key)
...
def all_holidays_with_bbq(holiday_hash)
  ...
  holiday_hash.each do |season, season_hash|
    season_hash.each do |holiday, holiday_hash|
